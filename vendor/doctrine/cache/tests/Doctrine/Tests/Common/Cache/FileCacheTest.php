...
class FileCacheTest extends \Doctrine\Tests\DoctrineTestCase
{
  ...
  protected function setUp()
  {
    $this->driver = $this->getMock(
        ...
        array('doFetch', 'doContains', 'doSave'), array(), '', false
    );
  }
  ...
  public function testFileExtensionCorrectlyEscaped()
  {
    $driver1 = $this->getMock(
        'Doctrine\Common\Cache\FileCache',
        array('doFetch', 'doContains', 'doSave'), array(__DIR__, '.*')
    );
    $driver2 = $this->getMock(
        'Doctrine\Common\Cache\FileCache',
        array('doFetch', 'doContains', 'doSave'), array(__DIR__, '.php')
    );
  ...
  public function testNonIntUmaskThrowsInvalidArgumentException()
  {
    ...
    $this->getMock(
        'Doctrine\Common\Cache\FileCache',
        array('doFetch', 'doContains', 'doSave'), array('', '', 'invalid')
    );
  }
  ...
  public function testGetDirectoryReturnsRealpathDirectoryString()
  {
    ...
    $driver    = $this->getMock(
        'Doctrine\Common\Cache\FileCache',
        array('doFetch', 'doContains', 'doSave'), array($directory)
    );
  ...
  public function testGetExtensionReturnsExtensionString()
  {
    ...
    $driver    = $this->getMock(
        'Doctrine\Common\Cache\FileCache',
        array('doFetch', 'doContains', 'doSave'), array($directory, $extension)
    );
   ...
   */
  public function testWindowsPathLengthLimitationsAreCorrectlyRespected($length,
                                                                        $pathShouldBeHashed)
  {
    if (!defined('PHP_WINDOWS_VERSION_BUILD')) {
      define('PHP_WINDOWS_VERSION_BUILD',
          'Yes, this is the "usual suspect", with the usual limitations');
    }

    $basePath = $this->getBasePathForWindowsPathLengthTests($length);

    $fileCache = $this->getMockForAbstractClass(
        'Doctrine\Common\Cache\FileCache', array($basePath, '.doctrine.cache')
    );
    ...
    $this->assertEquals(
        $length, strlen($keyPath),
        sprintf('Path expected to be %d characters long is %d characters long',
            $length, strlen($keyPath))
    );
    ...
    if ($pathShouldBeHashed) {
      $this->assertSame(
          $hashedKeyPath, $getFileName->invoke($fileCache, $key),
          'Keys should be hashed correctly if they are over the limit.'
      );
    } else {
      $this->assertSame(
          $keyPath, $getFileName->invoke($fileCache, $key),
