...
abstract class BaseFileCacheTest extends CacheTest
{
   ...
   */
  public function testWindowsPathLengthLimitIsCorrectlyHandled($length,
                                                               $pathShouldBeHashed)
  {
    ...
    $this->assertEquals($length, strlen($keyPath),
        "Unhashed path should be of correct length.");
    ...
    if ($pathShouldBeHashed) {
      $this->assertFileExists($hashedKeyPath,
          "Path generated for key should be hashed.");
      unlink($hashedKeyPath);
    } else {
      $this->assertFileExists($keyPath,
          "Path generated for key should not be hashed.");
      unlink($keyPath);
    }
  }
}