...
abstract class CacheTest extends \Doctrine\Tests\DoctrineTestCase
{
  ...
  public function testFetchMultiple()
  {
    ...
    $this->assertEquals(
        $saved, $cache->fetchMultiple($keys),
    ...
    $this->assertEquals(
        array_slice($saved, 0, 2), $cache->fetchMultiple($keysWithNonExisting),
  ...
  public function testNoCacheIdCollisions()
  {
    ...
    foreach ($ids as $index => $id) {
      ...
      $this->assertNotFalse($value,
          sprintf('Failed to retrieve data for cache id "%s".', $id[0]));
      if ($index !== $value) {
        $this->fail(sprintf('Cache id "%s" collides with id "%s".', $id[0],
                $ids[$value][0]));
      }
    }
  }
  ...
  public function testLifetime()
  {
    ...
    $this->assertTrue($cache->contains('expire'),
        'Data should not be expired yet');
  ...
  public function testNoExpire()
  {
    ...
    // @TODO should more TTL-based tests pop up, so then we should mock the `time` API instead
    sleep(1);
    $this->assertTrue($cache->contains('noexpire'),
        'Data with lifetime of zero should not expire');
  }
  ...
  public function testLongLifetime()
  {
    ...
    $this->assertTrue($cache->contains('longlifetime'),
        'Data with lifetime > 30 days should be accepted');
  }
